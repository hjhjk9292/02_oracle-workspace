...
JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
JOIN JOB USING(JOB_CODE)
WHERE SUBSTR(EMP_NO, 1, 4) BETWEEN '1970' AND '1979'
  AND SUBSTR(EMP_NO, 8, 1) = '2' -- 여자인 조건
  AND EMP_NAME LIKE '전%';
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, HIRE_DATE, 
       (SALARY + SALARY * BONUS) * 12 AS "보너스포함연봉"
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
...
--SELECT EMP_NAME
--FROM EMPLOYEE
--WHERE DEPT_CODE = (SELECT DEPT_CODE FROM EMPLOYEE WHERE EMP_ID);
...
FROM (
    SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, HIRE_DATE, 
           (SALARY + NVL(BONUS, 0)) * 12 AS "보너스포함연봉",  -- 보너스 포함 연봉 계산
           RANK() OVER (ORDER BY (SALARY + NVL(BONUS, 0)) * 12 DESC) AS 순위
    FROM EMPLOYEE
    JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
    JOIN JOB USING(JOB_CODE)
    ) 
...
SELECT DEPT_TITLE, SUM(SALARY) AS DEPT_SALARY_SUM
FROM EMPLOYEE
JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
GROUP BY DEPT_TITLE
HAVING SUM(SALARY) > (
    SELECT 0.2 * SUM(SALARY) 
    FROM EMPLOYEE
);
