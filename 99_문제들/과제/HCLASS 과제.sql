...
--A-3. 25살이상 고객님들의 구매한 상품명 조회하기 (강주찬) => 테이블 3개
...
SELECT CUSTOMID, PRICE, PCODE
FROM TBL_BUY
JOIN TBL_PRODUCT USING(PCODE)
ORDER BY CUSTOMID, PRICE ;
...
SELECT CUSTOMID, QUANTITY AS "평균 개수"
FROM TBL_BUY
JOIN TBL_PRODUCT USING(PCODE)
WHERE PRICE >= 10000;
...
SELECT PNAME, PCODE, PRICE * QUANTITY, QUANTITY -- 총 판매개수 COUNT 사용하려면 
FROM TBL_BUY
JOIN TBL_PRODUCT USING(PCODE)
WHERE SUBSTR(BUY_DATE, 1,2) = '23'
ORDER BY PCODE;

--SELECT PNAME, PCODE, "총 판매액", "총 판매개수" -- 총 판매개수 COUNT 사용하려면 
--FROM ( SELECT PNAME, PCODE, SUM(PRICE * QUANTITY) AS "총 판매액", COUNT(QUANTITY) "총 판매개수"
--        FROM TBL_BUY 
--        JOIN TBL_PRODUCT USING(PCODE)
--        GROUP BY PCODE
--        HAVING SUBSTR(BUY_DATE, 1,2) = '23')
...
SELECT AGE , PCODE "제품코드"
FROM TBL_BUY
JOIN TBL_PRODUCT USING(PCODE)
JOIN TBL_CUSTOM ON(CUSTOMID = CUSTOMID)
WHERE PNAME LIKE '%진라면%';

--SELECT AVG(AGE) , PCODE "제품코드"
--FROM TBL_BUY
--WHERE PNAME LIKE '%진라면%'(SELECT AGE , PCODE "제품코드"
--                FROM TBL_BUY
--                JOIN TBL_PRODUCT USING(PCODE)
--                JOIN TBL_CUSTOM ON(CUSTOMID = CUSTOMID)
--                WHERE PNAME LIKE '%진라면%'
