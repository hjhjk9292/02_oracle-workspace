...
-- 문제점 : 
-- 1. OR 연산자와 AND 연산자가 나열되어 있을 경우 AND 연산자 먼저 수행이 됨! 문제에서 요구한 내용으로 OR 연산이 먼저 수행되어야함!
-- 2. 급여값에 대한 비교가 잘못되어있음. >이 아닌 >= 으로 비교해야됨
-- 3. 보너스가 있는 이라는 조건에 IS NULL이 아닌 IS NOT NULL로 비교해야됨!!
-- 4. 여자에 대한 조건이 누락되어있음.
-- 5. 이메일에 대한 비교시 네번째 자리에 있는 _를 데이터값으로 취급하기 위해서는 새 와일드 카드를 제시해야되고, 또한 ESCAPE OPTION으로 등록까지 해야함!
